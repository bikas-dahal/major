...
train_transform = transforms.Compose([
    ...
    transforms.Normalize(
        ...
        std=[0.229, 0.224, 0.225]         # values from the ImageNet dataset
    )
])

# Define transformations for validation/testing data
val_transform = transforms.Compose([
    transforms.Resize(32),               # Resize the image to 32x32
    transforms.CenterCrop(32),           # Crop the center of the image
    transforms.ToTensor(),               # Convert the image to a PyTorch tensor
    transforms.Normalize(
